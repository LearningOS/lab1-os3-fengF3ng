...
impl TaskManager {
    ...
    fn check_start_time(&self, task_id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        if let None = inner.tasks[task_id].start_time {
            inner.tasks[task_id].start_time = Some(get_time());
        }
        drop(inner);
    }
    fn get_syscall_info(&self) -> [u32; MAX_SYSCALL_NUM] {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        let res = inner.tasks[current].syscall_times.clone();
        drop(inner);
        res
    }
